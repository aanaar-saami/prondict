    ...
    */
    ...
    if( node.hasChildNodes() ) {
		...
		if (trgLabel == "lemma") {
		    ...
		    switch (nAttrType) {
		    case "WFNR":
			nAttrValue = (nAttrValue.charAt(0).toUpperCase() + nAttrValue.slice(1)).replace('_p','/P');
			spanElement.setAttribute("class","wfattr");
			break;
		    case "WFCS":
			nAttrValue = nAttrValue.charAt(0).toUpperCase() + nAttrValue.slice(1) + ' of '
			spanElement.setAttribute("class","wfattr");
		    	break;
		    case "WFLM":
			spanElement.setAttribute("class","wflemma");
		    	break;
		    }
		    var textE = document.createTextNode(nAttrValue + " ");
		    spanElement.appendChild(textE);
